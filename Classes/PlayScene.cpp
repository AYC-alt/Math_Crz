...
void PlayScene::update(float dt)
{
	...
	_timeLabel->setString(StringUtils::format("%.2f", _timer));
...
void PlayScene::randomMath()
{
	...
	int _operator = random(0, 2);						//0 là trừ, 1 là cộng, 2 la nhan
	...
	case 1:
	{
		...
		if (_result)
		{
			...
			_mathStr = StringUtils::format("%d + %d\n=%d", _firstNum, _secondNum, _firstNum + _secondNum);
		}
		else
		{
			//_mathStr = String::createWithFormat("%d + %d\n=%d", _firstNum, _secondNum, random(_firstNum, _firstNum + _secondNum - 1))->getCString();
			_mathStr = StringUtils::format("%d + %d\n=%d", _firstNum, _secondNum, random(_firstNum, _firstNum + _secondNum - 1));
		}
		break;
	}
	case 0:
	{
		...
		else
		{
			...
			_mathStr = StringUtils::format("%d - %d\n=%d", _firstNum, _secondNum, random(_firstNum - _secondNum + 1, _firstNum + _secondNum));
		}
		break;
	}
	case 2:
